...
const DisplayCloseDueDateData = ({ jsonData }) => {
  ...
  useEffect(() => {
    ...
    const filteredData = jsonData.filter((item) => {
      ...
      return daysDifference <= 14 && daysDifference >= 7;
    });
  ...
  return (
    <div>
      <h2 className="pt-2 pb-2 mb-2 text-xl font-bold text-center bg-yellow-300 rounded-md shadow-xl">
        โครงการที่ใกล้ครบกำหนดระยะเวลาประกันผลงาน ในช่วงระยะ 8-14 วัน มีจำนวน{" "}
        ...
        {closeDueDateData.map((item) => (
          ...
          >
            <Link href={`/contract/${item.SEQ}`}>
              ...
              <p>
                จ<strong>จำนวนเงินในสัญญา (รวม VAT) :</strong>{" "}
